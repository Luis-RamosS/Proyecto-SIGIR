...
public class VentasPanel extends javax.swing.JPanel {
    ...
    public VentasPanel(){
        ...
        configurarComponentes(); aplicarEstilos(); configurarEventos();
        FiltroTiempoReal.activar(txtBuscarProducto,controlador::filtrarProductos);
        FiltroTiempoReal.activar(txtBuscarHistorial,controlador::buscarVentas);
        controlador.iniciar();
    }
    ...
    private void configurarComponentes(){
        ...
        cmbCliente.setRenderer(new DefaultListCellRenderer(){ @Override public java.awt.Component getListCellRendererComponent(JList<?> l,Object v,int i,boolean s,boolean f){ super.getListCellRendererComponent(l,v,i,s,f); if(v instanceof Cliente c)setText(c.getNombreCompleto()==null?"":c.getNombreCompleto()); return this; }});
        cmbProducto.setRenderer(new DefaultListCellRenderer(){ @Override public java.awt.Component getListCellRendererComponent(JList<?> l,Object v,int i,boolean s,boolean f){ super.getListCellRendererComponent(l,v,i,s,f); if(v instanceof Producto p)setText(p.getIdProducto()<=0?(p.getNombre()==null?"":p.getNombre()):(p.getCodigo()+" - "+p.getNombre())); return this; }});
    ...
    private void configurarEventos(){
        cmbProducto.addActionListener(e->controlador.seleccionarProducto()); btnAgregarProducto.addActionListener(e->controlador.agregarProducto()); btnQuitarProducto.addActionListener(e->controlador.eliminarProducto()); btnNuevaVenta.addActionListener(e->controlador.nuevaVenta()); btnGuardarVenta.addActionListener(e->controlador.registrarVenta());
    ...
    public void cargarClientes(List<Cliente> clientes){ Cliente sel=getClienteSeleccionado(); DefaultComboBoxModel<Cliente> m=new DefaultComboBoxModel<>(); Cliente op=new Cliente(); op.setIdCliente(0); op.setNombreCompleto("Seleccione un cliente..."); m.addElement(op); clientes.forEach(m::addElement); cmbCliente.setModel(m); if(sel!=null)for(int i=0;i<m.getSize();i++)if(m.getElementAt(i).getIdCliente()==sel.getIdCliente())cmbCliente.setSelectedIndex(i); }
    public void cargarProductos(List<Producto> productos){ int id=getProductoSeleccionado()==null?0:getProductoSeleccionado().getIdProducto(); DefaultComboBoxModel<Producto> m=new DefaultComboBoxModel<>(); Producto op=new Producto(); op.setIdProducto(0); op.setCodigo(""); op.setNombre(productos.isEmpty()?"No hay coincidencias":"Seleccione un producto..."); m.addElement(op); productos.forEach(m::addElement); cmbProducto.setModel(m); for(int i=0;i<m.getSize();i++)if(m.getElementAt(i).getIdProducto()==id)cmbProducto.setSelectedIndex(i); }
    public Cliente getClienteSeleccionado(){ Object v=cmbCliente.getSelectedItem(); return v instanceof Cliente c?c:null; }
    public Producto getProductoSeleccionado(){ Object v=cmbProducto.getSelectedItem(); return v instanceof Producto p?p:null; }
    public String getTextoBusquedaProducto(){ return txtBuscarProducto.getText().trim(); }
    ...
    public void limpiarVenta(){ if(cmbCliente.getItemCount()>0)cmbCliente.setSelectedIndex(0); txtFechaVenta.setText(LocalDate.now().format(FORMATO_FECHA)); txtUsuario.setText(Sesion.haySesionActiva()?Sesion.getNombreCompleto():""); cmbMetodoPago.setSelectedItem("EFECTIVO"); txtMontoRecibido.setText("0.00"); txtCambio.setText("0.00"); txtObservaciones.setText(""); txtMotivoDescuento.setText(""); txtFechaVencimiento.setText(""); txtMontoCuota.setText(""); limpiarProductoSeleccionado(); tabsVentas.setSelectedIndex(0); configurarMetodoPago(); }
    public void limpiarProductoSeleccionado(){ txtBuscarProducto.setText(""); if(cmbProducto.getItemCount()>0)cmbProducto.setSelectedIndex(0); txtCantidad.setText("1"); txtPrecioVenta.setText("0.00"); txtStockDisponible.setText("0"); txtDiasGarantia.setText("0"); lblAvisoSerie.setText("Selecciona un producto para ver sus datos."); }
    public void establecerProcesando(boolean p){btnGuardarVenta.setEnabled(!p);btnAgregarProducto.setEnabled(!p);btnQuitarProducto.setEnabled(!p);btnNuevaVenta.setEnabled(!p);btnGuardarVenta.setText(p?"Guardando...":"Guardar / Cobrar");}
    ...
    private void initComponents(){
        ...
        lblTituloAgregarProducto=new JLabel("Agregar producto"); lblBuscarProducto=new JLabel("Filtro"); txtBuscarProducto=new JTextField(); lblProducto=new JLabel("Producto"); cmbProducto=new JComboBox<>(); lblCantidad=new JLabel("Cantidad"); txtCantidad=new JTextField("1"); lblPrecioVenta=new JLabel("Precio"); txtPrecioVenta=new JTextField("0.00"); lblStockDisponible=new JLabel("Stock"); txtStockDisponible=new JTextField("0"); lblDiasGarantia=new JLabel("Garantía (días)"); txtDiasGarantia=new JTextField("0"); btnAgregarProducto=new JButton("+ Agregar"); lblAvisoSerie=new JLabel("Selecciona un producto para ver sus datos.");
